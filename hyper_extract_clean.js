...
async function main() {
  if (!fs.existsSync(cleanZip)) {
    console.error(`repo_clean.zip not found at: ${cleanZip}`);
    process.exit(1);
  }

  console.log('Opening local repo_clean.zip with unzipper...');
  const directory = await unzipper.Open.file(cleanZip);
  console.log(`Zip contains ${directory.files.length} entries. Commencing sequential buffer extraction...`);
  ...
  console.log(`Done writing files sequentially. Total files written: ${writtenCount}`);
